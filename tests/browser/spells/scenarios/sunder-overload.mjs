import { runSunderFallbackScenarios } from './sunder-fallbacks.mjs';
import { runSunderWidenedScenarios } from './sunder-widened.mjs';
...
export async function runSunderOverloadScenarios(suite) {
  ...
  /* The widened strike arranges its own game, so nothing above flows into it —
     but it must still run BEFORE the fallbacks. Their restart beat asserts that
     a fresh game inherits no SUNDER presentation, and what it inherits from is
     whatever cast last touched this page. Keep both awaits in this order. */
  await runSunderWidenedScenarios(suite);
  await runSunderFallbackScenarios(suite);
}
