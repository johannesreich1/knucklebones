...
try {
  ...
  try {
    ...
    /* 4b · THE NATIVE BUTTON OPENS THE NATIVE SHARE SHEET WITH A PUBLIC LINK.
       The native builds serve the same bundle from https://localhost, so
       location.origin there is a private address. The old implementation
       could only copy from WKWebView; even that copied a link to the
       recipient's own machine (reported from a device 2026-08-30). The
       harness supplies the same Capacitor bridge iOS injects and deliberately
       leaves navigator.share present: the native plugin must win. */
    ...
    check(shared.skipped || (shared.seen.native.length === 1 && shared.seen.web.length === 0),
      'THE NATIVE SHARE BUTTON DID NOT USE THE CAPACITOR SHARE SHEET', shared);
    check(shared.skipped || !/\/\/(localhost|127\.0\.0\.1)/.test(
      shared.seen.native[0]?.url ?? ''),
      'THE NATIVE SHARE LINK POINTS AT LOCALHOST — that is the recipient\'s '
      + 'own machine, so the shared link goes nowhere', shared);
    check(shared.skipped || /^https:\/\/[a-z0-9.-]+\//.test(shared.seen.native[0]?.url ?? ''),
      'the shared link is not a public https URL', shared);
    check(shared.skipped || (shared.seen.native[0]?.title
      && shared.seen.native[0].dialogTitle === shared.seen.native[0].title
      && shared.seen.native[0].text),
      'the native sheet lost its localized title/dialog title or result text', shared);

    /* Closing the native sheet is a completed interaction, not a signal to
       open a second share UI underneath it. Capacitor iOS rejects with the
       official `Share canceled` message; no web or clipboard fallback may
       fire for that one branch. */
    ...
    out.dismissedShare = dismissedShare;
    ...
    /* A broken or missing native plugin is different: the same tap must still
       reach the web share API. This is the installed-old-binary fallback that
       keeps a newly deployed web bundle usable before the next native build. */
    ...
    out.failedNativeShare = failedNativeShare;
