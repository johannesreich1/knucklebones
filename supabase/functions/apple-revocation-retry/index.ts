...
import { createServiceClient, json, postOnly, withErrorBoundary } from "../_shared/http.ts";
...
Deno.serve(withErrorBoundary(async (request) => {
  ...
  if (!expected
    || !(await secretEquals(request.headers.get("X-Knucklebones-Cron") ?? "", expected))) {
  ...
  if (error || !Array.isArray(data)) {
    console.error("apple revocation claim failed:", error?.message);
  ...
  for (const row of data as Array<Record<string, unknown>>) {
    ...
    if (!expired) {
      try {
        ...
        console.error("apple revocation attempt failed:", revokeError);
        result = "retry";
      }
    }
    ...
    if (finishError) {
      console.error("apple revocation finish failed:", finishError.message);
      return json({ error: "finish-failed" }, 500);
    }
