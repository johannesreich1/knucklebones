-- The history RPC asks for DESC NULLS LAST, while the previous DESC indexes
-- used PostgreSQL's default NULLS FIRST. Make the physical order match the API
-- and express complete cursors as a row comparison so each branch can use one
-- contiguous keyset range.

drop index if exists public.matches_p1_history_idx;
drop index if exists public.matches_p2_history_idx;

create index matches_p1_history_idx
  on public.matches (p1, season_id, finished_at desc nulls last, id desc)
  where status <> 'active';

create index matches_p2_history_idx
  on public.matches (p2, season_id, finished_at desc nulls last, id desc)
  where status <> 'active';
  ...
  with history as (
    select m.id,
     ...
     where m.p1 = auth.uid()
       ...
       and (
         before_t is null
         or (before_id is null and m.finished_at < before_t)
         or (before_id is not null and (m.finished_at, m.id) < (before_t, before_id))
       )
