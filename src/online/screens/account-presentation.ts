...
export function paintAccountFrame(
  ...
  const apex = standing
    ? inApex(ladder.points, standing.rank, standing.population)
    : false;
  const ring = $('#accRing') as HTMLElement;
  const peakPosition = groupRingPeakState(ladder.points, ladder.peak, apex);
  fillAccountRing(ring, groupRingFill(ladder.points, apex));
