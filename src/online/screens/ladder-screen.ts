...
export function createLadderScreen(ports: LadderPorts): LadderScreen {
  ...
  async function show(): Promise<void> {
    ...
    let rows: LadderRow[];
    if (me && standing?.rank) {
      const prior = await ladderPageBefore(ABOVE_ME, standing.rank, me.nickname);
      topDry = prior.length < ABOVE_ME;
      const anchor = prior[prior.length - 1];
      const tail = anchor
        ? await ladderPage(PAGE - prior.length, Number(anchor.rank), anchor.nickname)
        : await ladderPage(PAGE, standing.rank);
      rows = [...prior, ...tail];
    } else {
      rows = await ladderPage(PAGE);
    }
    ...
    const growDown = (): void => {
      ...
      loading = true;
      void ladderPage(PAGE, bottomCursor.rank, bottomCursor.nickname).then((page) => {
        loading = false;
        if (run !== showRevision || !isOnlinePanelCurrent('onLadder')) return;
    ...
    const growUp = (): void => {
      ...
      loading = true;
      void ladderPageBefore(PAGE, topCursor.rank, topCursor.nickname).then((page) => {
        loading = false;
        if (run !== showRevision || !isOnlinePanelCurrent('onLadder')) return;
        const fresh = page.filter((row) => !seen.has(row.nickname));
