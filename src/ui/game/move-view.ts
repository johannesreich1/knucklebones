...
import { burst, flash, floatPts, shake } from '../fx.ts';
import { colorOf, heatOf } from '../identity.ts';
import { renderSide } from './board.ts';
import { shieldBlocked, wardBurned } from './seals.ts';
...
export async function animateGameMove(
  ...
  if (!spec.isCurrent()) return { placed: false, interrupted: true, destroyed: 0 };
  ...
  if (!spec.isCurrent()) return { placed: true, interrupted: true, destroyed: 0 };
