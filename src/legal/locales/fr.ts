  ...
  pages: {
    ...
    privacy: {
      ...
      sections: [
        ...
        { heading: 'Destinataires, régions et transferts', blocks: [p('Supabase fournit l’authentification, la base de données, les Edge Functions et Realtime. La région de la base est {{supabaseDatabaseRegion}} et celle des Edge Functions est {{supabaseFunctionsRegion}}.'), p('Cloudflare Pages distribue la PWA hébergée. Le périmètre de traitement concerné est : {{cloudflareProcessingScope}}.'), p('Sur iOS, les options Connexion avec Apple et Game Center transmettent via les services Apple des identifiants de compte Apple ou de joueur d’équipe et des éléments de vérification signés. La vérification Game Center passe par un Worker Cloudflare à débit limité avant Supabase ; l’application ne reçoit pas d’autres données du profil Game Center.'), p('Les garanties applicables aux transferts internationaux sont : {{transferSafeguards}}. L’application native charge plutôt les fichiers web inclus dans son paquet.'), p('Nous n’intégrons aucun SDK publicitaire ou d’analyse comportementale ni script distant de marketing ou d’analyse. Les fournisseurs d’infrastructure peuvent néanmoins créer des journaux opérationnels, de sécurité et d’accès.')] },
