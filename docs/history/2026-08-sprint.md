# August 2026 sprint history

> Archived snapshot of the detailed build narrative as it stood on
> 2026-08-22. It preserves the measurements, rejected approaches, rollout
> lessons, and live-verification notes earned during the sprint. It is not the
> source for current deployment versions or open work; use
> [`docs/STATUS.md`](../STATUS.md) for that.
...
| **Backend** | Supabase project `euzjcejbkxvqfrttgaxu` (EU) — schema through migration 0024, RLS + column-grant hardened. The then-named 0014 Game Center migration (later timestamped `20260826153100_game_center_ids.sql` before rollout) is written but NOT applied — it waits for a device. 0028 (player_card returns the full face-off row — renumbered from 0027, which the nickname hardening took) applied 2026-08-21: the result screen's foe plate carries the rank and opens the face-off |
...
it. Gated by `tests/rune-deal-reveal.mjs`.
...
- `flow/spells.ts` — the runtime orchestration and charges (`S.spellCharges`),
  with gestures in `flow/spell-gestures.ts` and cast animation in
  `flow/spell-effects.ts`. Every input path funnels through one `cast()`, so
  legality is asked exactly once and a spell can never half-happen.
...
(same trap as the `.dying` survivors in single-strike-visibility). Gates:
`tests/spells.test.ts`
...
that made the swap degenerate (earned timing, not sniping). The measurements
above used the then-current charm-blind, one-cast-per-turn simulator; they are
historical floors rather than results for the later scoring WARD policy. The
one-cast limit remains the live rule; the old charm blindness does not.
...
tap the die); `EFFECTS` in `flow/spell-effects.ts` is the per-spell animation
...
- **Never lower the game-finish loop budgets** in widget-isolation,
  responsive-browser and tutorial-persistence (900–1200 ticks). Three CI flakes
  proved 300–400 is not enough.
...
- **One gate at a time per working tree, but any number across trees.** Since
  ...
  `pwa-update` over `pwa/`. Contention is the leftover: with several gates
...
Gated by **`tests/profile-back-navigation.mjs`** (served: the online chunk is
lazy), which walks
...
destination is the same drift the HUD's sliders glyph was (§6).
`tests/result-screen.mjs` pins the stack: exactly two buttons, the second
labelled and **shorter in
...
- **Nothing rendered the cards.** The build catches what Node can see; a card
  ...
  thirteen were — `01-widths` was losing 811px of itself.
  `tests/design-cards-render.mjs`
