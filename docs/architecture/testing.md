...
### The app ships its own face, and every suite checks it bound

`src/styles/foundations/typeface.css` bundles Chakra Petch (OFL 1.1, four
weights, latin + latin-ext) and `page.css` names it first. Before that the app
shipped no font at all: the stack named OS faces, so every pixel a suite
measured depended on what the host owned. `system-ui` is a real system face on
macOS, Android and Windows; on Linux it is whatever fontconfig ranks first,
which nobody chose. A stock GitHub runner resolved it to DejaVu Sans and the
bare Playwright image to WenQuanYi Zen Hei, so hosted CI measured a rendering no
player would ever see and reported its metrics as layout defects — design cards
12-16px "too tall", a French legal title "wrapped". Two commits answered that by
widening card frames, which baked those metrics into source.

With one face bundled the runner and the phone lay out the same pixels, so a
geometry number finally means the same thing in both places. What still falls
through to the OS is `ja`/`ko` (no latin face carries CJK) and
`design/build.mjs`'s springboard mock, which asks for `system-ui` deliberately —
which is why the Roboto step below stays.
...
`tests/support/rendering-font.mjs` is the guard, and bundling a face made it
stronger: it now asks whether OUR family loaded at every weight we declare and
whether the stack bound it, which is answerable on every platform — a broken
asset pipeline or a family-name typo fails locally instead of waiting for the
runner. It must be called on a page that HAS the app's stylesheet; asked on
about:blank it correctly reports that the family does not exist.
`design-cards-render` and the localization runner call it before any geometry,
so a font failure names itself instead of emitting a list of things that are
12px too tall and sending the reader off to edit card declarations.
`tests/typeface.test.ts` is the static half: no rule may carry a literal weight,
and no rung of the ladder may sit above the heaviest face we ship. Do not answer a font-shaped geometry failure by widening a budget
