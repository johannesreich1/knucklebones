...
|---|---|---|---|
| `ranked-runes` | `pvp-rune-select`, `pvp-action`, `account-delete`, `pvp-claim`, `pvp-move`, `pvp-join` | `KB_ALLOW_PRODUCTION_RANKED_RUNE_FUNCTIONS` | all five pinned forward migrations through `20260901162456_progression_v2`, the exact stage-3 catalog/data audit, and the complete Rune Trial foundation |
...
- **Its own database prerequisite** (apply only, never in preview). The
  ...
  `ranked-runes` requires all five exact migration-history identities through
  `20260901162456_progression_v2` and stage 3, including the persisted RANDOM
  column/trigger/helpers, the exact historical-Silver v3 body, and the
  invariant that bots retain canonical fixed seats with RANDOM off.
